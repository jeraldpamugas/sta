...
@section('content')
    <div id="divHome" class="row">
        {{-- <div style="margin: 20px;">
            <button style="border-radius: 20px;" class="btn"><i class="fas fa-map-marker-alt"></i> Portland <i
        ...
        @if ($usertype != 'staff')
            <div class="col-sm-6" style="margin-bottom: 10px;">
                <div class="card pendingCard">
                    @if ($usertype == 'supervisor')
                        <h3
                            style="background-color: rgba(255, 0, 0, 0.5); margin: 0; padding: 10px; color: white; text-align: center;">
                            Pending for Authorization</h3>
                    @else
                        <h3
                            style="background-color: rgba(255, 0, 0, 0.5); margin: 0; padding: 10px; color: white; text-align: center;">
                            Pending for Confirmation</h3>
                    @endif
                                ...
                                @if ($trans->id)
                                    @if ($trans->isOpened)
                                        <a class="pendingTransA" href="{{ route('transactions.edit', $trans->id) }}">
                                            <div style="padding: 1px 10px; box-shadow: inset 5px 0 0 0 rgb(172, 112, 0);"
                                                class="card-body">
                                                <span style="height: 100%; width:10px; background-color: limegreen;"></span>
                                                <h4 class="card-title"><i class="far fa-envelope-open"></i> Transaction No:
                                                    {{ $trans->transNo }}
                                                </h4>
                                    ...
                                    @else
                                        <a class="pendingTransA" href="{{ route('transactions.edit', $trans->id) }}">
                                            <div style="padding: 1px 10px; box-shadow: inset 5px 0 0 0 rgb(158, 0, 0);"
                                                class="card-body">
                                                <span style="height: 100%; width:10px; background-color: limegreen;"></span>
                                                <h4 class="card-title"><b><i class="fa fa-envelope" aria-hidden="true"></i>
                                                        Transaction No: {{ $trans->transNo }}</b></h4>
            ...
            <div class="col-sm-6" style="margin-bottom: 10px;">
                <div class="card upcomingCard">
                    @if ($usertype == 'supervisor')
                        <h3
                            style="background-color:  rgba(0, 158, 71, 0.5); margin: 0; padding: 10px; color: white; text-align: center;">
                            Authorized</h3>
                        <div style="overflow-x: hidden; height: 270px;">
                            @if (!$transactionsCP)
                                <div style="padding: 1px 10px;" class="card-body">
        ...
        <div class="col-sm-12">
            ...
            <div class="col-sm-3">
                @if ($usertype == 'staff')
                    <a style="display: block;" class="btn btn-success" href="{{ route('transactions.create') }}">New
                        Transaction</a>
                @endif
            </div>
            <div class="col-sm-12" style="overflow-y: hidden; height: 100%; margin: 10px 0;">
                <table style="border-collapse: collapse; width: 100%;" id="transactionTable"
                    ...
                    <tbody>
                        @foreach ($transactionsToday as $trans)
                            <tr>
                                <td><a class="confirmedTrans"
                                        href="{{ route('transactions.show', $trans->id) }}"><b>{{ $trans->transNo }}</b></a>
                                </td>
