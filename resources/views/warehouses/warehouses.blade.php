...
<div class="row">
  <div class="col-12 text-right">
      <div class="col-sm-6" style="float: left;">
           ...
           <a href="javascript:void(0)" style="" class="btn btn-success mb-3 pull-left" id="create-new-post" onclick="addPost()">Add Warehouse</a>
       </div>
  </div>
</div>
     ...
     <table id="warehouseTable" class="table table-striped table-hover">
       ...
       <tbody>
           ...
           <tr id="row_{{$warehouse->id}}">
              ...
              <td>
               <a href="javascript:void(0)" data-id="{{ $warehouse->id }}" onclick="editPost(event.target)" class="btn btn-info" style="width: 45%; padding: 6px 6px;"><i data-id="{{ $warehouse->id }}" onclick="editPost(event.target)" class="fas fa-edit"></i></a>
               <a href="javascript:void(0)" data-id="{{ $warehouse->id }}" onclick="deletePost(event.target)" class="btn btn-danger" style="width: 45%; padding: 6px 6px;"><i data-id="{{ $warehouse->id }}" onclick="deletePost(event.target)" class="fas fa-trash-alt"></i></a></td>
           </tr>
           @endforeach
       </tbody>
     </table>
                ...
                <div class="row">
                   <div class="col-xs-12 col-sm-12 col-md-12">
                       ...
                       <div class="form-group">
                           ...
                           <input type="text" id="warehouseCode" name="warehouseCode" class="form-control" placeholder="Input Warehouse Code">
        ...
        success: function(response) {
            ...
            if(response.code == 200) {
              ...
              if(id != ""){
                  ...
                  $('table tbody').prepend('<tr id="row_'+res.data.id+'"><td>'+res.data.id+'</td><td>'+res.data.warehouseCode+'</td><td>'+res.data.Description+'</td><td><a href="javascript:void(0)" data-id="'+res.data.id+'" onclick="editPost(event.target)" class="btn btn-info" style="width: 45%; padding: 6px 6px;"><i data-id="'+res.data.id+'" onclick="editPost(event.target)" class="fas fa-edit"></i></a><a href="javascript:void(0)" data-id="'+res.data.id+'" class="btn btn-danger" onclick="deletePost(event.target)" style="width: 45%; padding: 6px 6px;"><i data-id="'+res.data.id+'" onclick="deletePost(event.target)" class="fas fa-trash-alt"></i></a></td></tr>');
              }
